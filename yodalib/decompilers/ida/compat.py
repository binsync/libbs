...
@execute_write
@requires_decompilation
def set_function_header(yodalib_header: yodalib.data.FunctionHeader, exit_on_bad_type=False, ida_code_view=None):
    data_changed = False
    func_addr = ida_code_view.cfunc.entry_ea
    ...
    if yodalib_header.name and yodalib_header.name != cur_ida_func.name:
        set_ida_func_name(func_addr, yodalib_header.name)
    ...
    types_to_change = {}
    for idx, yodalib_arg in yodalib_header.args.items():
        ...
        # change the name
        if yodalib_arg.name and yodalib_arg.name != cur_ida_arg.name:
            success = ida_code_view.rename_lvar(ida_code_view.cfunc.arguments[idx], yodalib_arg.name, 1)
            data_changed |= success

        # record the type to change
        if yodalib_arg.type and yodalib_arg.type != cur_ida_arg.type:
            types_to_change[idx] = (cur_ida_arg.type, yodalib_arg.type)
...
def set_stack_vars_types(var_type_dict, ida_code_view) -> bool:
    ...
    @param controller:          The yodalib controller to do operations on
    @return:
    """
